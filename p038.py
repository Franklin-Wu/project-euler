...
# Returns the pandigital string if it is pandigital; returns None if it is not.
...
for permutation in permutations:
    string = test_2_term_sum(int("94" + permutation));
    if string != None:
        solutions.append(int(string));

permutations = get_permutations(["8765421"], 2);
for permutation in permutations:
    string = test_2_term_sum(int("93" + permutation));
    if string != None:
        solutions.append(int(string));

permutations = get_permutations(["8765431"], 2);
for permutation in permutations:
    string = test_2_term_sum(int("92" + permutation));
    if string != None:
        solutions.append(int(string));

permutations = get_permutations(["8765432"], 2);
for permutation in permutations:
    string = test_2_term_sum(int("91" + permutation));
    if string != None:
        solutions.append(int(string));

print "solutions = %s" % solutions;
print "max = %d" % max(solutions);
